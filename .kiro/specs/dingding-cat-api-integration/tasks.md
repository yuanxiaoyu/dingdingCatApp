...
- [x] 22. 优化广告交互体验和收益计算逻辑
  - **实现广告事件完整API集成**
    - 在Mock模式下，为广告观看、点击、完播等事件添加完整的API接口请求
    - 集成 /ad/show（展示）、/ad/click（点击）、/ad/complete（完播）、/ad/skip（跳过）接口调用
    - 确保每个广告事件都能正确触发对应的API请求并处理响应
  - **实现广告按钮状态管理**
    - 广告播放期间显示加载状态，禁用按钮防止重复点击
    - 广告播放完成后恢复按钮正常状态，允许下次播放
    - 添加播放进度指示器和状态反馈
  ...
  - **重构收益展示逻辑**
    ...
    - 改为统计和显示用户广告点击次数
    - 实现收益提示：完播次数 × 单次收益金额
    - 从风控配置接口(/config/risk)获取单次收益金额配置
    - 显示格式："您已完成 X 次广告观看，预计收益 ¥X.XX"
  - **移除Banner广告功能**
    - 从主界面移除Banner广告按钮
    - 清理相关的Banner广告组件和逻辑
    - 更新界面布局，重新排列剩余的广告类型按钮
  - **完善Mock数据支持**
    - 更新Mock服务支持新的收益计算逻辑
    - 模拟风控接口返回单次收益金额配置
    - 确保所有广告事件的Mock响应正确
  - _需求: 4.1, 4.2, 4.3, 4.4, 4.5, 6.1, 6.2, 9.2, 9.3_
