...
- [x] 21. 实现开发环境Mock功能和开屏广告流程
  ...
  - 实现开屏广告与穿山甲SDK的完整集成
  - **配置Mock模式模拟用户信息加载成功**
  - **修复首页用户信息加载失败问题，确保Mock用户数据正常显示**
  - **实现首页顶部Banner广告位展示**
  - **完整API模拟功能实现**
    - 模拟用户认证接口(/auth/wechat/login, /auth/wechat/register)返回正常登录数据
    - 模拟应用配置接口(/config)返回基础配置信息
    - 模拟广告配置接口(/config/ad)返回广告位配置，基于adConfig.js设置
    - 模拟风控配置接口(/config/risk)返回风控规则配置
    - 模拟渠道配置接口(/config/channel)返回渠道相关配置
    - 模拟用户信息接口(/user/info)返回完整用户资料
    - 模拟收益统计接口(/ad/revenue)返回用户收益数据
    - 模拟广告请求接口(/ad/request)返回各类型广告数据
    - 确保所有Mock数据与真实API响应格式完全一致
    - 根据adConfig.js配置控制广告加载行为和显示状态
    - 实现Mock数据的持久化存储和状态管理
    - 添加Mock模式切换和调试工具
    - 验证应用启动流程：初始化→认证→配置加载→广告展示→首页显示
  - **使用Mock数据验证应用逻辑的完整性**
  - **确保开屏广告完播后能正确自动跳转到首页**
  - **验证首页Banner广告位能正常加载和点击**
